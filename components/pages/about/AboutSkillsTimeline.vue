<template>
  ...
  >
    <div
      ref="stage"
      class="sticky top-0 h-screen w-full bg-white z-10 overflow-hidden"
    >
      <div
        ref="barsWrap"
        class="about-skills-bars absolute inset-x-1 md:inset-x-6 top-0 left-0 pt-28 lg:pt-36 pb-6 h-full flex items-center"
      >
        <div class="w-full h-full flex flex-col gap-2">
          ...
          >
            ...
            >
              ...
              >
                <div
                  class="h-full flex items-center whitespace-nowrap overflow-hidden text-ellipsis"
                >
                  <span
                    class="inline-flex items-center min-w-full px-1 md:px-3 py-1 text-xs lg:text-sm font-mono bg-white border rounded-md overflow-visible"
                  >
                    {{ s.label }}
                  </span>
                </div>
      ...
      <div
        v-reveal
        class="absolute top-14 left-6 right-6 sm:left-10 sm:right-10 md:left-16 md:right-16"
      >
        <div class="relative h-16">
          ...
          >
            ...
            <div
              class="-translate-y-[35px] md:translate-y-0 mt-2 text-xs md:text-lg md:text-xl font-semibold text-black"
            >
              {{ y }}
            </div>
          </div>
...
onMounted(async () => {
  ...
  ctx = gsap.context(() => {
    barEls.value = Array.from(
      stage.value!.querySelectorAll<HTMLElement>('.about-skills-bar')
    )
    fillEls.value = Array.from(
      stage.value!.querySelectorAll<HTMLElement>('.about-skills-fill')
    )
    tickEls.value = Array.from(
      stage.value!.querySelectorAll<HTMLElement>('.about-skills-tick')
    )
          ...
          if (r >= 1) {
            ...
            if (contentEl) {
              ...
              if (minWidthPct > 100) {
                ...
                const clampedExtraProgress = Math.min(
                  1,
                  Math.max(0, extraProgress)
                )
