<template>
  <div class="min-h-screen from-slate-50 to-slate-200 overflow-hidden relative">
    <div
      ref="timelineWrapper"
      class="sticky top-0 h-screen flex items-start pt-20 flex items-center"
    >
      <h2
        v-reveal
        class="font-[Neutral] text-lg uppercase absolute top-[60px] left-6 z-[3]"
      >
        {{ $t('about.timeline.title') }}
      </h2>

      <div ref="timelineTrack" class="timeline-track px-6 z-[1]">
        <div class="grid grid-cols-[repeat(13,_400px)] gap-3 mb-8">
          <div
          ...
          >
            ...
            <div
              v-if="step.skills?.length > 0"
              v-reveal-group
              class="w-full max-w-md"
            >
              <p class="text-sm font-medium text-gray-800 mb-2 text-center">
                {{ $t('about.timeline.skills') }}
              </p>
              <div v-reveal-group class="flex flex-wrap justify-center gap-1">
                <span
        ...
        <div class="grid grid-cols-[repeat(52,_91px)] gap-3">
          <template
            v-for="(step, year, yearIndex) in timeline"
            :key="`milestones-${year}`"
          >
            <div
            ...
            >
              ...
              <h4 class="text-sm text-gray-700 mb-1">
                {{ $t(`about.timeline.milestones.${milestone.key}.subtitle`) }}
              </h4>
              <p
                v-if="
                  $te(`about.timeline.milestones.${milestone.key}.location`)
                "
                class="text-xs text-gray-500 mb-2"
              >
                {{ $t(`about.timeline.milestones.${milestone.key}.location`) }}
              </p>

              <div
                v-if="
                  $te(`about.timeline.milestones.${milestone.key}.projects`)
                "
                class="mb-2"
              >
                <div
                  v-for="(project, key) in $tm(
                    `about.timeline.milestones.${milestone.key}.projects`
                  )"
                  :key="key"
                  class="text-xs text-gray-700 mb-1 flex items-start"
                >
                  <span
                    class="inline-block w-1 h-1 bg-gray-400 rounded-full mt-1.5 mr-1 flex-shrink-0"
                  />
                  {{ project }}
                </div>
              </div>
              ...
              <div
                v-if="$te(`about.timeline.milestones.${milestone.key}.bullets`)"
                class="space-y-1"
              >
                <div
                  v-for="(bullet, key) in $tm(
                    `about.timeline.milestones.${milestone.key}.bullets`
                  )"
                  :key="key"
      ...
      <div
        class="absolute bottom-8 left-1/2 transform -translate-x-1/2 text-center text-gray-500 z-[2] pointer-events-none"
      >
        <div class="text-xs mb-2">{{ $t('about.timeline.scroll') }}</div>
        <div
          class="w-6 h-10 border-2 border-gray-300 rounded-full mx-auto relative"
        >
          <div
            class="w-1 h-3 bg-gray-300 rounded-full absolute top-2 left-1/2 transform -translate-x-1/2 animate-bounce"
          />
        </div>
      </div>

      <div v-reveal class="absolute top-0 left-0 w-full h-full flex z-[0] p-3">
...
const timeline = {
  ...
  2018: {
    skills: [
      ...
      'Zeplin',
      'CSS Animations',
      'Front in PHP env (Drupal & Symfony)'
    ],
    milestones: [
      {
        key: 'edu_bachelor_hetic',
        start: 2,
        end: 6,
        row: 1,
        type: 'education'
  ...
  2021: {
    ...
    milestones: [
      { key: 'xp_faume_lead', start: 1, end: 20, row: 2, type: 'experience' },
      {
        key: 'xp_shopify_consultant',
        start: 3,
        end: 5,
        row: 1,
        type: 'experience'
  ...
  2026: {
    skills: ['Stripe Connect', 'Supabase', 'Next Cache'],
    milestones: [
      { key: 'xp_daresonly', start: 0, end: 3, row: 1, type: 'experience' }
    ]
  }
}
