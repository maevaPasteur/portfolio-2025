<template>
  <div class="min-h-screen from-slate-50 to-slate-200 overflow-hidden relative">
    <div ref="timelineWrapper" class="sticky top-0 h-screen flex items-start pt-20 flex items-center">

      <h2 v-reveal class="font-[Neutral] text-lg uppercase absolute top-[60px] left-6 z-[3]">
      ...
      <div ref="timelineTrack" class="timeline-track px-6 z-[1]">
        ...
        <div class="grid grid-cols-[repeat(12,_400px)] gap-3 mb-8">
          ...
          >
            ...
            <div v-if="step.skills?.length > 0" v-reveal-group class="w-full max-w-md">
        ...
        <div class="grid grid-cols-[repeat(48,_100px)] gap-3">
          <template v-for="(step, year, yearIndex) in timeline" :key="`milestones-${year}`">
            ...
            >
              ...
              <p
                v-if="$te(`about.timeline.milestones.${milestone.key}.location`)"
              ...
              <div v-if="$te(`about.timeline.milestones.${milestone.key}.projects`)" class="mb-2">
                <div
                  v-for="(project, key) in $tm(`about.timeline.milestones.${milestone.key}.projects`)"
                  :key="key"
                  class="text-xs text-gray-700 mb-1 flex items-start">
                  <span class="inline-block w-1 h-1 bg-gray-400 rounded-full mt-1.5 mr-1 flex-shrink-0"/>
                  {{ project }}
                </div>
              </div>
              ...
              <div v-if="$te(`about.timeline.milestones.${milestone.key}.bullets`)" class="space-y-1">
                <div
                  v-for="(bullet, key) in $tm(`about.timeline.milestones.${milestone.key}.bullets`)"
                  :key="key"
      ...
      <div class="absolute bottom-8 left-1/2 transform -translate-x-1/2 text-center text-gray-500 z-[2] pointer-events-none">
        ...
        <div class="w-6 h-10 border-2 border-gray-300 rounded-full mx-auto relative">
          <div class="w-1 h-3 bg-gray-300 rounded-full absolute top-2 left-1/2 transform -translate-x-1/2 animate-bounce"/>
        </div>
      </div>
