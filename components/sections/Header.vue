<template>
  <header class="font-mono fixed z-20 top-0 left-0 w-full text-white mix-blend-difference">
    <div class="flex items-center pl-6 pr-3 min-h-12">
      ...
      <button class="ml-auto md:hidden flex items-center justify-center text-2xl min-w-10 min-h-10" @click.prevent="isMenuOpened = true">
        <Icon name="mdi:menu"/>
      </button>
      <nav class="ml-auto hidden md:block text-xs uppercase">
        <ul class="flex">
          <li v-for="(page, i) in menu" :key="i" class="py-4 px-3 flex items-center">
            <NuxtLink
