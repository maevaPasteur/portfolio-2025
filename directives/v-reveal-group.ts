...
export const vRevealGroup: Directive<
  HTMLElement,
  RevealGroupOptions | undefined
> = {
  mounted(el, binding) {
    const {
      threshold = 0.1,
    ...
    observer.observe(el)
    ;(
      el as HTMLElement & { __revealGroupObserver?: IntersectionObserver }
    ).__revealGroupObserver = observer
  },
  unmounted(el) {
    const obs: IntersectionObserver | undefined = (
      el as HTMLElement & { __revealGroupObserver?: IntersectionObserver }
    ).__revealGroupObserver
